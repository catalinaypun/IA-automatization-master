...
|-------|-------------------|-------------------------------------------------------------------|-----------------------------------------------------------------------|
| US-01 | Promotion Manager | view a promotion list with key fields and status                  | I can quickly understand active offers and their commercial setup      |
| US-02 | Promotion Manager | inspect product-level quantity tiers within each promotion         | I can validate pricing behavior by min/max quantity ranges             |
...
- **Then** I see promotions grouped into a collapsible accordion, each item showing the promotion name, date range, bundle count, and status badge, collapsed by default except the first entry

### US-02 — Product quantity tier visibility
- **Given** I expand a promotion in the accordion
- **When** the promotion has more than one child bundle (e.g. different term lengths)
- **Then** I can switch between bundles using a segmented control, and each bundle shows its own Terms/Free Months/Lock Price/Discount Type/Install values and a single consolidated product table with min quantity, max quantity, and price per product — never side-by-side duplicate tables per product
...
|------------------|-------------------------------------------|-------------------------------------------------------------------|
| default          | Page opens with valid data                | Promotions accordion is visible, first item expanded; Add Promotion can open the builder view |
| loading          | Promotion data retrieval in progress      | Loading indicator is visible; promotions accordion is hidden      |
| validation-error | User submits incomplete required data     | Validation message appears while the accordion remains visible    |
| system-error     | Promotions retrieval fails                | System error message is shown; promotions accordion is hidden     |
...
- [ ] Should the accordion allow multiple promotions expanded at once (current behavior), or should expanding one auto-collapse the others?
